...
> **Status: M0 complete — Nami runs inside a real Unity game.** The native core (injector +
> loader) hosts a bundled .NET 10 runtime inside the game process; the chainloader loads mods
> in isolated ALCs with crash quarantine; the sample `HelloNami` mod runs its update loop
> inside Project Hardline (Unity 2022.3.27f1, Mono) with the game stable. The experimental
> CoreCLR→Mono reverse-interop bridge is opt-in (`enableMonoBridge` in nami.json) pending GC
> interop hardening.
...
src/
  ...
  Nami.Patch/      Detour-based patch engine                [later milestone]
