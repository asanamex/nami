...
BepInEx-Mono loads plugins **into the game's own embedded Mono** - an old runtime
with an old BCL, GC, and JIT. Plugins are stuck with all three.
...
BepInEx-IL2CPP's first launch runs a Cpp2IL + generator pipeline and preloads the
generated interop assemblies. Nami's IL2CPP backend calls the game's own
...
with per-type lazy materialization in-process.
...
| Harmony-style method patching | yes (HarmonyX) | **Wave** - in-house detours + IL-copy patching and cursor transpilers: `Wave.Patch` routes prefix-only hooks on small primitive signatures to a native fast stub, everything else (postfix, transpilers, instance, refs) to IL-copy prefix/postfix for any closed method with a real body (incl. `calli`; open generics via `Wave.Patch(definition, typeArguments, ...)`), skip, `__instance`/`__result`/`__state`/`__args` |
