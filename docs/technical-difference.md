...
| Injection surface | Doorstop (separate project, C++). | In-repo C++ (`native/`, ~7.6k non-blank lines): injector + loader. `nami_loader.dll` is fully static-linked (no MinGW runtime DLLs to resolve in a foreign process); `nami_boot.exe` is MinGW-linked. |
